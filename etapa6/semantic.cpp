...
bool isCompatible(int type1, int type2)
{
    ...
    else if (type1 == SYMBOL_DATA_TYPE_BOOL)
    {
        if (type2 == SYMBOL_DATA_TYPE_BOOL)
            compatible = true;
    }
    return compatible;
}
    ...
    case AST_VECTORDEC_INIT:
        if (node->symbol)
        {
            if (node->symbol->type == SYMBOL_IDENTIFIER)
            {
                ...
                {
                    std::string reversed = reverseString(node->children[1]->symbol->text);
                    long unsigned int idx = stoi(reversed);
                    if (idx == 0)
                    {
...
void checkUsage(AST *node)
{
    ...
    case AST_SYMBOL:
        if (node->symbol)
        {
            ...
            {
                fprintf(stderr, "Semantic error at line %d: variable '%s' used but not declared\n", node->lineNumber, node->symbol->text.c_str());
                semantic_errors++;
            }
        }
        break;
...
void checkTypes(AST *node, int expectedFuncReturnType)
{
    if (!node)
        return;
    ...
    case AST_FUNDEC:
    {
        if (node->children.size() == 3)
        {
            ...
            expectedFuncReturnType = returnType;
        }
        break;
    }
    ...
    case AST_RETURN:
    {
        if (node->children.size() == 1)
        {
            ...
            if (!isCompatible(expectedFuncReturnType, returnType))
            {
    ...
    case AST_WHILE:
    {
        if (node->children.size() == 2)
        {
            ...
            if (condType != SYMBOL_DATA_TYPE_BOOL)
            {
    ...
    for (auto child : node->children)
        checkTypes(child, expectedFuncReturnType);
}
...
int getExprType(AST *node)
{
    ...
    case AST_NE:
    {
        ...
        if ((left == SYMBOL_DATA_TYPE_INT && right == SYMBOL_DATA_TYPE_INT) ||
            ...
            (left == SYMBOL_DATA_TYPE_INT && right == SYMBOL_DATA_TYPE_CHAR) ||
            (left == SYMBOL_DATA_TYPE_CHAR && right == SYMBOL_DATA_TYPE_CHAR) ||
            (left == SYMBOL_DATA_TYPE_REAL && right == SYMBOL_DATA_TYPE_REAL))
            return SYMBOL_DATA_TYPE_BOOL;
        return SYMBOL_DATA_TYPE_INVALID;
    }
    case AST_AND:
    case AST_OR:
